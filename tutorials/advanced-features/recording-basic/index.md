...
This tutorial is a simple video-call application, built upon [Node.js server](https://livekit-tutorials.openvidu.io/tutorials/application-server/node/index.md) and [JavaScript client](https://livekit-tutorials.openvidu.io/tutorials/application-client/javascript/index.md) tutorials, and extends them by adding recording capabilities:
...
- When running OpenVidu in **Azure**, recordings are stored in an **Azure Blob Storage container**. If this is your case, follow the [Recording Basic Azure tutorial](https://openvidu.io/latest/docs/tutorials/advanced-features/recording-basic-azure/) on openvidu.io instead.
...
The server application extends the [Node.js server tutorial](https://livekit-tutorials.openvidu.io/tutorials/application-server/node/index.md) by adding the following REST API endpoints:
...
1. The path where recordings will be stored in the S3 bucket.
1. The portion size of the recording that will be sent to the client in each request. This value is set to `5 MB`.
1. Set the `public` directory as the static files location.
...
1. Extracts the recording name from the `fileResults` array.

1. Returns the recording metadata to the client.
...
1. The `getActiveRecordingByRoom` function retrieves the active recording for a room.
1. If there is no active recording for the room, the server returns a `409 Conflict` status code.
1. Stop the egress to finish the recording by calling the `stopEgress` method of the `EgressClient` with the egress ID (`activeRecording`) as a parameter.
1. Return the updated recording metadata to the client.

This endpoint does the following:

1. Obtains the `roomName` parameter from the request body. If it is not available, it returns a `400` error.
1. Retrieves all active egresses for the room. If there is no active egress for the room, it returns a `409` error to prevent stopping a non-existent recording.
1. Extracts the `egressId` from the active egress.
1. Stops the egress to finish the recording by calling the `stopEgress` method of the `EgressClient` with the egress ID (`activeRecording`) as a parameter.
1. Returns the updated recording metadata to the client.
...
1. List all Egress video files in the S3 bucket.
1. Map all of the recording names from the S3 response.
1. Filter the recordings by room ID, if available.
1. Return the list of recordings to the client.

This endpoint does the following:

1. Obtains the `roomId` query parameter for later filtering, if available.
1. Lists all Egress video files in the S3 bucket. To accomplish this, we use the `listObjects` method of the `S3Service` with the `RECORDINGS_PATH` parameter.
1. Extracts the recording names from the S3 response.
1. Filters the recordings by room ID, if available. The room ID is part of the recording name, so we can filter with a quick check.
1. Returns the list of recordings to the client.
...
1. Get the recording file from the S3 bucket.
1. Set the response status code to `206 Partial Content`.
1. Set the `Cache-Control` header as `no-cache`.
1. Set the `Content-Type` header as `video/mp4`.
1. Set the `Accept-Ranges` header as `bytes`.
1. Set the `Content-Range` header with the start and end of the recording file and its size.
1. Set the `Content-Length` header as the size of the recording file portion.
1. Pipe the recording file to the response.

This endpoint does the following:

1. Extracts the `recordingName` parameter from the request.

1. Checks if the recording exists in the S3 bucket by calling the `exists` method of the `S3Service` with the `key` as a parameter. If the recording does not exist, it returns a `404` error.
...
1. Gets the requested range of the recording file by calling the `getRecordingStream` function:
   ...
   1. Get the start of the requested range.
   1. Get the end of the requested range or set it to the start plus the established portion size.
   1. Get the minimum between the end of the requested range and the size of the recording file minus one.
   1. Get the recording file from the S3 bucket with the requested range.
   ...
   1. Extracts the start of the requested range from the `range` header.
   1. Extracts the end of the requested range from the `range` header. If the end is not provided, it sets the end to the start plus the established portion size.
   1. Gets the minimum between the end of the requested range and the size of the recording file minus one. This is done to prevent requesting a range that exceeds the recording file size.
   1. Gets the recording file from the S3 bucket with the requested range by calling the `getObject` method of the `S3Service` with the `key` and `range` as parameters.
...
1. Check if the recording exists in the S3 bucket.
1. Delete the recording file from the S3 bucket.

This endpoint does the following:

1. Extracts the `recordingName` parameter from the request.
1. Checks if the recording exists in the S3 bucket by calling the `exists` method of the `S3Service` with the `key` as a parameter. If the recording does not exist, it returns a `404` error.
1. Deletes the recording file from the S3 bucket by calling the `deleteObject` method of the `S3Service` with the object's `key` as a parameter.
...
1. The access key of the S3 server.
1. The secret key of the S3 server.
1. The AWS region of the S3 server.
1. The name of the S3 bucket.
1. Initialize the `S3Client` with the provided configuration.
1. Check if an object exists in the S3 bucket.
1. Retrieve the metadata of an object in the S3 bucket.
1. Retrieve the size of an object in the S3 bucket.
1. Retrieve a specified range of bytes from an object in the S3 bucket.
1. List objects in the S3 bucket that match a regex pattern.
1. Delete an object from the S3 bucket.
1. Execute an S3 command.
