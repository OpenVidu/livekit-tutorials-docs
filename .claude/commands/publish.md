...
2. Run the [Publish Web](https://github.com/OpenVidu/livekit-tutorials-docs/actions/workflows/publish-web.yaml)
   ...
   It runs `mkdocs gh-deploy --strict --force`; GitHub Pages then rebuilds `gh-pages` a little
   later (`gh api repos/OpenVidu/livekit-tutorials-docs/pages/builds/latest -q .status` →
   `built`).
3. **If the change touched the tutorials, openvidu.io needs publishing too**, or the two copies
   ...
   `latest`.
4. Verify live: the changed pages, `robots.txt` and `llms.txt` (200; `llms.txt` opens with
   "# LiveKit Tutorials" and lists 27 entries), the footer year, the "Running OpenVidu?" box
   under a tutorial's H1, `og:title` in a page's head, and that `/overrides/main.html`
   still 404s.
