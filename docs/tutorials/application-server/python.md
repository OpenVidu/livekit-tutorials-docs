...
3. We set the video grants in the AccessToken. `room_join` allows the user to join a room and `room` determines the specific room. Check out all [Video Grants :fontawesome-solid-external-link:{.external-link-icon}](https://openvidu.io/latest/docs/reference/access-tokens/#video-grants){:target="_blank"}.
...
2. Initialize a `WebhookReceiver` using the `TokenVerifier`. It will help validating and decoding incoming [webhook events :fontawesome-solid-external-link:{.external-link-icon}](https://openvidu.io/latest/docs/reference/webhooks/){:target="_blank"}.
