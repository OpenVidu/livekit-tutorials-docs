...
[counterpart]: https://openvidu.io/latest/docs/tutorials/application-server/node/?utm_source=livekit-tutorials&utm_medium=referral&utm_campaign=tutorial-cross-link
...
It also imports the `WebhookReceiver` class, used further down to validate and decode incoming [webhook events :fontawesome-solid-external-link:{.external-link-icon}](https://openvidu.io/latest/docs/reference/webhooks/){:target="_blank"}.
...
2. We set the video grants in the AccessToken. `roomJoin` allows the user to join a room and `room` determines the specific room. Check out all [Video Grants :fontawesome-solid-external-link:{.external-link-icon}](https://openvidu.io/latest/docs/reference/access-tokens/#video-grants){:target="_blank"}.
