...
- `shared/` snippets render inside several pages; grep for the snippet's `--8<--` usages before
  editing one.
- `hooks/mkdocs_hook.py` stamps the copyright year. Add build-time computation there.
- Pins live in `Dockerfile` and `.github/workflows/publish-web.yaml` and **must match
  openvidu.io's** (mkdocs 1.6.1, material 9.7.6, pymdown-extensions 11.0.1, glightbox 0.5.2):
  the two sites render the same Markdown, so a version drift is a rendering drift.
